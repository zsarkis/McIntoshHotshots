    ...
    private async Task<string> ExecuteFinishingAttemptsFromValueFunction(string argumentsJson, string userId, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var result = JsonSerializer.Serialize(new
            {
                ...
                fastest_successful_finish = analysis.FastestSuccessfulFinish > 0 ? (int)analysis.FastestSuccessfulFinish : (int?)null,
                slowest_successful_finish = analysis.SlowestSuccessfulFinish > 0 ? (int)analysis.SlowestSuccessfulFinish : (int?)null,
